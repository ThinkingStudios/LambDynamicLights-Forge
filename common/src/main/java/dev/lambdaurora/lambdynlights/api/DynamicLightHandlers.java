...
public final class DynamicLightHandlers {
	...
	public static void registerDefaultHandlers() {
		...
		registerDynamicLightHandler(EntityType.ITEM,
				entity -> RyoamicLights.getLuminanceFromItemStack(entity.getStack(), entity.isSubmergedInWater()));
		...
		registerDynamicLightHandler(EntityType.GLOW_ITEM_FRAME, entity -> {
			var world = entity.getWorld();
			return Math.max(14, RyoamicLights.getLuminanceFromItemStack(entity.getHeldItemStack(),
	...
	public static <T extends Entity> int getLuminanceFrom(T entity) {
		if (!RyoamicLights.get().config.getEntitiesLightSource().get())
			return 0;
		if (entity == MinecraftClient.getInstance().player && !RyoamicLights.get().config.getSelfLightSource().get())
			return 0;
	...
	public static <T extends BlockEntity> int getLuminanceFrom(T entity) {
		if (!RyoamicLights.get().config.getBlockEntitiesLightSource().get())
			return 0;
