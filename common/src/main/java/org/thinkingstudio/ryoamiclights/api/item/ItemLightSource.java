...
public abstract class ItemLightSource {
	...
	public int getLuminance(ItemStack stack, boolean submergedInWater) {
		if (this.waterSensitive() && RyoamicLights.get().config.getWaterSensitiveCheck().get() && submergedInWater)
	...
	public static @NotNull Optional<ItemLightSource> fromJson(@NotNull Identifier id, @NotNull JsonObject json) {
		if (!json.has("item") || !json.has("luminance")) {
			RyoamicLights.get().warn("Failed to parse item light source \"" + id + "\", invalid format: missing required fields.");
		...
		if (luminanceElement.isNumber()) {
			...
			RyoamicLights.get().warn("Failed to parse item light source \"" + id + "\", invalid format: \"luminance\" field value isn't string or integer.");
		}
