...
public final class DynamicLightHandlers
{
    ...
    public static void registerDefaultHandlers()
    {
        ...
        registerDynamicLightHandler(EntityType.ITEM, entity -> RyoamicLights.getLuminanceFromItemStack(entity.getStack(), entity.isSubmergedInWater()));
    ...
    {
        if (!RyoamicLights.get().config.hasBlockEntitiesLightSource())
            return 0;
        DynamicLightHandler<T> handler = (DynamicLightHandler<T>) getDynamicLightHandler(entity.getType());
        if (handler == null)
            return 0;
